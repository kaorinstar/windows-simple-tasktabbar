...
対象は、ユーザーが定義したグループだけです。アプリ1つで1グループになっているものは、名前から
決まる色をそのまま使うため、定義済みグループと同じ色になることがあります。これらも対象に
するには何らかの並び順が必要ですが、利用できる順序はどれも開いているウィンドウに依存します。
それでは、ウィンドウを開閉するたびに色が変わってしまいます。
