...
最小化されたウィンドウも対象にしますが、絵が出ない場合があります。コンポジターは画面にある
ものを描くため、最小化されたウィンドウには描く中身がありません。タスクバーがそれを表示できる
のは、シェルが他のアプリに公開されていない経路を持っているためです。最小化されたウィンドウが
縮小画像を持つための公開された仕組み（`DWMWA_HAS_ICONIC_BITMAP` と
`WM_DWMSENDICONICTHUMBNAIL`）は、そのウィンドウの持ち主が使うもので、外から見る側は使えません。

そこで、絵の領域に先にウィンドウのアイコンを描き、その上にコンポジターへ描かせます。描かれる
ウィンドウならアイコンは隠れ、描かれないウィンドウならアイコンが残ります。最小化かどうかを
判定する箇所はどこにもなく、どちらでも空の枠にはなりません。
