...
前面にあるウィンドウのタブは、他のタブより少し高く描き、輪郭線を付けます。どちらも描画だけの
違いです。`tab.Bounds` はすべてのタブで同じなので、幅の計算、当たり判定、ドラッグ、スクロールは
変わりません。タブの中身も高くした形ではなく `tab.Bounds` を基準に配置するため、ウィンドウが
前面に来ても文字の位置は動きません。
...
高くする分は `BarMetrics.TopOffset`、つまり行の上に空いている帯から取ります。ここには他に何も
描かれないため、どのタブも場所を譲りません。帯のうち1画素は残し、バー上端の線が途切れないように
します。描画の切り取り範囲は、行の上端ではなくバーの上端からにします。ただし `_contentRect` は
そのままです。これはクリック判定に使うもので、帯はどのタブのものでもないためです。

幅を広げる案は検討して見送りました（#9）。`TabStrip.Measure` は行全体に1つの幅を返し、4か所が
