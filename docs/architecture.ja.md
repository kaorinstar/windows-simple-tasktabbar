...
**グループを動かした直後は、ポインターがタブ1枚分進むまで次の移動を止めます。** 判定は
`TabStrip.MovedFarEnough` です。これは省いてよい処理ではありません。`DropIndex` は
ポインターの位置だけを見るため、「このグループが相手の前にある」状態と「後ろにある」状態を
分けているのは、1ピクセルの境目だけです。手を置いているだけでもこの境目を何度もまたぐので、
実機では相手グループがタブ2枚分の距離を左右に往復して見えました。タブ1枚のドラッグで
同じことが起きないのは、掴んでいるタブ自身がポインターの下のスロットに入るため、元へ戻る
までに半タブ分の余裕ができるからです。塊で動かすとその余裕がないので、ここで作っています。
グループの中での移動は、もともと余裕があるため止めません。
