...
リリースのタグは日付で付けます。形式は `vYYYY.M.D` で、先頭に0を付けません。`v2026.09.06`
ではなく `v2026.9.6` です。同じ日に2回目を出すときは、`v2026.9.6.1` のように4つ目の数字を
足します。
...
説明文には `version.md` の該当する節が入ります。形式が違うタグや、`version.md` に節がない
