...
- **表示言語を選ぶ** — 設定で切り替えます。Windows に従う、英語、日本語の3つです。
  初期状態は Windows に従う設定です。翻訳がまだない表示言語では英語で表示します。書体も
  言語に合わせて変わり、日本語は Yu Gothic UI、英語は Segoe UI で表示します。
...
- **実行ファイル名が同じ別のアプリは同じグループになります。** ユーザーが見分けやすい
  ...
  1つのアプリとして扱われます。
- **画面の表示は英語と日本語だけです。** 他に10言語を追加する予定です（[#35](https://github.com/kaorinstar/windows-simple-tasktabbar/issues/35)）。
  それまでは、対応していない表示言語では英語で表示します。
