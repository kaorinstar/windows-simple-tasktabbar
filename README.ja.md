...
- **タブの上で右クリック** — 閉じる・ほかのタブを閉じる・左側のタブを閉じる・
  右側のタブを閉じる・最小化・このアプリを除外のメニューを表示します。
...
- **アプリをバーから除外する** — 設定で指定します。バーから切り替えることのない常駐ツールや
  チャットのアプリを外しておくと、実際に使うタブが広くなります。ウィンドウは閉じられず、
  タブだけがなくなります。タブを右クリックすれば1手順で除外できます。除外したアプリは、
  起動中のアプリと並べて設定に表示されるため、あとから元に戻せます。起動していないアプリも、
  名前を入力して除外できます。
- **アプリの並び順を決める** — 設定で指定します。左側に置きたいアプリを、置きたい順に並べて
  おくと、新しく開いたウィンドウは行の末尾ではなくその位置に入ります。一覧にないアプリのタブは
  右端に付きます。これはこれまでのすべてのタブと同じ動きです。一覧を変えるとすぐに並べ替わり、
  そのあともタブをドラッグして自由に動かせます。グループ化を有効にしている場合は、この一覧
  1つで、グループどうしの順番とグループの中の順番の両方が決まります。
...
- **バーを置く位置を選ぶ** — 設定で切り替えます。タスクバーに合わせる、画面の下、画面の上、
  の3つです。初期状態はタスクバーに合わせる設定です。Windows のタスクバーを画面の上へ移すと、
  設定も再起動もせずにバーも上へ移ります。タスクバーが左右にあるときは、タブが横一列に
  並ぶため、バーは画面の下に表示します。
- **すべてのディスプレイにバーを表示する** — それぞれのバーには、そのディスプレイ上の
  ウィンドウだけが並びます。ウィンドウをもう一方のディスプレイへ移すと、タブもすぐに
  そちらのバーへ移ります。最大化したウィンドウは、そのディスプレイのバーの手前で止まります。
  表示倍率はディスプレイごとに読み取るため、100% と 150% のように異なる設定でも正しい
  大きさで表示します。ディスプレイを増やすとバーが増え、外すとそのバーがなくなります。
  設定で「メイン ディスプレイのみ」を選ぶと、バーは1つだけになり、すべてのウィンドウが
  そのバーに並びます。
...
- **表示言語を選ぶ** — 設定で切り替えます。Windows に従うほか、12言語から選べます。英語・
  ...
  各言語の語句は、Windows 自体が同じものを指すときに使う語に合わせています。内容を確認できて
  いるのは英語と日本語の2つです。他の言語の訳の誤りはお知らせください。
...
![2列に並んだ設定画面。左列に言語・バーの高さ・配色・タブの並び順・ウィンドウのプレビュー、右列にバーの位置・ディスプレイ・タブのグループ・除外するアプリ・更新の確認](docs/images/settings.png)

## ダウンロード

[リリース](https://github.com/kaorinstar/windows-simple-tasktabbar/releases)には3つ添付しています。
...
- **`WindowsSimpleTaskTabBar-Setup-<バージョン>.exe`** — インストーラーです。スタートメニューの
  ...
  画面の言語は、バーが対応する12言語のうち10言語に対応しています。残る2つは英語表示になります。
...
配布するビルドはこの1つだけです。.NET 8 を同梱した版は約69MBかつ x64専用で、利点があるのは
...
そのため管理者権限を求めることはありません。古い版の上に新しい版を入れる場合と削除する場合は、
バーが動いていると終了を求めるメッセージが出ます。バーの上で右クリックして「Exit」を選び、
「OK」を押してください。削除は「設定」→「アプリ」から行い、
スタートメニューの項目とスタートアップのショートカットも一緒になくなります。設定は
`%APPDATA%\WindowsSimpleTaskTabBar\settings.json` に残るため、入れ直しても引き継がれます。

**インストーラーを使わない場合**

1. 実行ファイルを任意のフォルダーに置きます（例：`C:\Tools\WindowsSimpleTaskTabBar\`）。
2. ダブルクリックで起動します。
3. Windows の起動時に自動で立ち上げる場合は、`Windows` + `R` キーで `shell:startup` を開き、
   実行ファイルのショートカットを置いてください。

インストーラーと実行ファイルのどちらにも署名していないため、初回の実行時に SmartScreen の
警告が出ます。「詳細情報」から実行してください。

終了は、バーの上で右クリックして「Exit」を選びます。

## 更新の確認

インストーラーも実行ファイルも自分自身を置き換えません。パッケージ管理の仕組みもないため、
新しい版が出たことをお知らせする手段がほかにありません。そこで、起動から少し経ったところで
`github.com` にある最新の版番号を読みに行きます。
...
- **ダウンロードも置き換えも行いません。** 新しい版があれば通知領域でお知らせし、メニューに
  **Update available: v0.6.0...** が現れます。選ぶとブラウザーでリリースページが開きます。
  新しい版の取得と差し替えは、導入したときと同じように利用者ご自身で行ってください。
  インストーラーを上から実行するか、実行ファイルを置き換えてください。
- **実行は1日1回までです。** 同じ版のお知らせは1回だけで、ログオンのたびに繰り返しません。
  メニューの項目は、古い版を使っている間は新しい版の番号を表示し続けます。次にバーを起動した
  ときも残ります。
...
インストーラーは、この実行ファイルを [Inno Setup](https://jrsoftware.org/isinfo.php) でまとめて
作ります。実行できるのは Windows のみです。バージョンはコマンドラインで渡します。渡さない場合は
コンパイルできません。
...
├── docs/                                設計の説明
├── installer/                           インストーラーの Inno Setup スクリプト
...
インストーラー・持ち運び用ZIP・実行ファイルの3つを作り、リリースに添付します。同じワークフローを
手動で実行した場合は、同じ3つをダウンロード可能なビルド成果物として出力するだけで、リリースは
作りません。タグを付ける前に動作を確認できます。
...
これでリリース用ワークフローがビルドとテストを行い、3つのファイルを作ってリリースを公開します。
説明文には `version.md` の該当する節が入ります。実行ファイルにはタグのバージョンが埋め込まれ、
インストーラーとZIPのファイル名にも同じ番号が入ります。
